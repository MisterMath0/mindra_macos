    ...
    func executeQuery(_ query: String, params: [Any] = []) throws -> [[String: Any]] {
        return try connection.performSync {
            ...
            do {
                ...
                if let db = self.connection.getDatabasePointer() {
                    let sqliteError = String(cString: sqlite3_errmsg(db))
                    let code = sqlite3_errcode(db)
                    throw DatabaseError.queryFailed("SQLite error \(code): \(sqliteError) - Query: \(query)")
                } else {
            ...
            do {
                ...
                if result != SQLITE_DONE {
                    if let db = self.connection.getDatabasePointer() {
                        ...
                        throw DatabaseError.queryFailed("SQLite step failed with code \(result)")
                    }
                }
            } catch {
                if let dbError = error as? DatabaseError {
                    throw dbError
                } else if let db = self.connection.getDatabasePointer() {
                    let errorMessage = String(cString: sqlite3_errmsg(db))
                    let errorCode = sqlite3_errcode(db)
                    throw DatabaseError.queryFailed("SQLite error \(errorCode): \(errorMessage) - Query: \(query)")
                } else {
